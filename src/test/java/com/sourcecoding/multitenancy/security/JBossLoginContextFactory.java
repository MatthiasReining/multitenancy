...
/**
 * Provides a {@link LoginContext} for use by unit tests. It is driven by
 * users.properties and roles.properties files as described in <a
 * href="https://community.jboss.org/wiki/UsersRolesLoginModule"
 * >UsersRolesLoginModule</a>
 */
public class JBossLoginContextFactory {
	...
	static class JBossJaasConfiguration extends Configuration {
		...
		/**
		 * The {@link org.jboss.security.auth.spi.UsersRolesLoginModule} creates
		 * the association between users and roles.
		 * 
		 * @return
		 */
		private AppConfigurationEntry createUsersRolesLoginModuleConfigEntry() {
			...
			return new AppConfigurationEntry(
					"com.sourcecoding.multitenancy.MultiTenancyDatabaseServerLoginModule",
					AppConfigurationEntry.LoginModuleControlFlag.REQUIRED,
					options);
		}

		/**
		 * The {@link org.jboss.security.ClientLoginModule} associates the user
		 * credentials with the {@link org.jboss.security.SecurityContext} where
	 ...
	 */
	public static LoginContext createLoginContext(final String tenant,
			final String username, final String password) throws LoginException {
		final String configurationName = "Arquillian Testing";

		CallbackHandler cbh = new JBossLoginContextFactory.NamePasswordCallbackHandler(
